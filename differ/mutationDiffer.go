...
func (d *MutationDiffer) Run() error {
	...
	for i := 0; i < d.numberOfWorkers; i++ {
		...
		diffWorker := NewDifferWorker(d, d.sourceDcpAgent, d.targetDcpAgent, d.sourceBucket, d.targetBucket, combinedFetchList[lowIndex:highIndex], waitGroup, d.colIdsMap, d.reverseTgtColIdsMap, d.migrationHintMap, d.compareBody)
		waitGroup.Add(1)
		go diffWorker.run()
	}
...
func NewDifferWorker(differ *MutationDiffer, sourceDCPAgent, targetDCPAgent *gocbcore.DCPAgent, sourceBucket, targetBucket *GocbcoreAgent, fetchList MutationDiffFetchList, waitGroup *sync.WaitGroup, colIds, reverseColIds map[uint32][]uint32, migrationHintMap MigrationHintMap, compareBody bool) *DifferWorker {
	return &DifferWorker{
		differ:           differ,
