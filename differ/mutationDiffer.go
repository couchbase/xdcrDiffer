...
func NewMutationDiffer(sourceClusterUUID, sourceBucketName, sourceBucketUUID string, sourceRef *metadata.RemoteClusterReference,
	targetClusterUUID, targetBucketName, targetBucketUUID string, targetRef *metadata.RemoteClusterReference,
	fileDifferDir string, mutationDifferFileDir string, numberOfWorkers int, batchSize int, timeout int, maxNumOfSendBatchRetry int,
	sendBatchRetryInterval time.Duration, sendBatchMaxBackoff time.Duration, compareType string, logger *xdcrLog.CommonLogger,
	colIdsMap map[uint32][]uint32, srcCapability metadata.Capability, tgtCapability metadata.Capability, xdcrUtils xdcrUtils.UtilsIface,
	retries int, retriesWaitSecs int, duplMapping DuplicatedHintMap, factory *file.Factory) *MutationDiffer {
	...
	return &MutationDiffer{
		...
		srcDiffKeysFileName:    utils.DiffKeysFileName(true, fileDifferDir, base.DiffKeysFileName, factory.GetSuffix()),
		tgtDiffKeysFileName:    utils.DiffKeysFileName(false, fileDifferDir, base.DiffKeysFileName, factory.GetSuffix()),
...
func (d *MutationDiffer) writeCollectionMapping() error {
	...
	srcMapFilename := d.mutationDifferFileDir + base.FileDirDelimiter + fileName + d.factory.GetSuffix()
	srcMapFile, err := d.factory.OpenFile(srcMapFilename, os.O_RDWR|os.O_CREATE|os.O_TRUNC, base.FileModeReadWrite, file.WriteMode)
	if err != nil {
		d.logger.Errorf("Unable to open collection mapping file: %v\n", srcMapFilename)
		return err
	}
	defer srcMapFile.Close()

	_, err = srcMapFile.Write(srcMappingBytes)
	if err != nil {
...
func (d *MutationDiffer) writeKeysWithError() error {
	...
	keysWithErrorFileName := d.mutationDifferFileDir + base.FileDirDelimiter + base.DiffErrorKeysFileName + d.factory.GetSuffix()
	keysWithErrorFile, err := d.factory.OpenFile(keysWithErrorFileName, os.O_RDWR|os.O_CREATE|os.O_TRUNC, base.FileModeReadWrite, file.WriteMode)
	if err != nil {
		d.logger.Errorf("Unable to open keysWithError file: %v\n", keysWithErrorFileName)
		return err
	}
...
func (d *MutationDiffer) writeDiffBytesToFile(diffBytes []byte) error {
	...
	fullFileName := d.mutationDifferFileDir + base.FileDirDelimiter + fileName + d.factory.GetSuffix()

	diffFile, err := d.factory.OpenFile(fullFileName, os.O_RDWR|os.O_CREATE|os.O_TRUNC, base.FileModeReadWrite, file.WriteMode)
	if err != nil {
		return err
	}
...
func (d *MutationDiffer) loadDiffKeys() (DiffKeysMap, DiffKeysMap, MigrationHintMap, error) {
	srcFile, err := d.factory.OpenFile(d.srcDiffKeysFileName, os.O_RDONLY, 0, file.ReadMode)
	if err != nil {
		return nil, nil, nil, err
	}
	srcDiffKeysBytes, err := srcFile.ReadAll()
	srcFile.Close()
	if err != nil {
		return nil, nil, nil, err
	}

	tgtFile, err := d.factory.OpenFile(d.tgtDiffKeysFileName, os.O_RDONLY, 0, file.ReadMode)
	if err != nil {
		return nil, nil, nil, err
	}
	tgtDiffKeyBytes, err := tgtFile.ReadAll()
	tgtFile.Close()
	if err != nil {
		return nil, nil, nil, err
	}
	...
	migrationHintFileName := utils.DiffKeysSrcMigrationFileName(d.srcDiffKeysFileName, d.factory.GetSuffix())
	migrationFile, err := d.factory.OpenFile(migrationHintFileName, os.O_RDONLY, 0, file.ReadMode)
	if err == nil {
		migrationHintBytes, err = migrationFile.ReadAll()
		migrationFile.Close()
		if err != nil {
			return nil, nil, nil, err
		}
...
func (d *MutationDiffer) writeMigrationDetails() error {
	...
	fd, err := d.factory.OpenFile(srcMapFilename, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0644, file.WriteMode)
	if err != nil {
