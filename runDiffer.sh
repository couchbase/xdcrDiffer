...
function printHelp() {
	...
	cat <<EOF
Usage: $0 -u <username> -p <password> -h <hostname:port> -s <sourceBucket> -t <targetBucket> -r <remoteClusterName> [-v <targetUrl>] [-n <remoteClusterUsername> -q <remoteClusterPassword>] [-c clean] [-b ] [-e <mutationRetries>] [-w <setupTimeoutInSeconds>] [-k <sourcePrivateKeyFile> ] [-f <TargetPrivateKeyFile>]
...
while getopts ":h:p:u:r:s:t:n:q:v:cbe:w:k:f:" opt; do
	case ${opt} in
	u)
		username=$OPTARG
		;;
	...
	w)
		...
		;;
	k)
		sourcePrivateKey=$OPTARG
		;;
	f)
		targetPrivateKey=$OPTARG
		;;
	\?)
...
if [[ ! -z "$sourcePrivateKey" ]]; then
	execString="${execString} -sourcePrivateKey"
	execString="${execString} $sourcePrivateKey"
fi
if [[ ! -z "$targetPrivateKey" ]]; then
	execString="${execString} -targetPrivateKey"
	execString="${execString} $targetPrivateKey"
fi
